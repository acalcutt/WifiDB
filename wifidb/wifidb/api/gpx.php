...
switch($func)
{
	case "exp_latest_ap":
		...
		$results = $dbcore->createGPX->CreateApFeatureCollection($ExportCurrentApArray['data']);
		if($labeled){$file_name = "Latest_Labeled.gpx";}else{$file_name = "Latest.gpx";}
	break;
	...
	case "exp_live_ap":
		...
		foreach($appointer as $ap)
		{
			...
			$Import_Map_Data .=$dbcore->createGPX->CreateApFeature($ap_info);
		}
		$results = $dbcore->createGPX->createGPXstructure($Import_Map_Data, $labeled);
	break;
	...
	case "exp_ap":
		...
		$results = $dbcore->createGPX->CreateApFeatureCollection($ApArray['data']);
		$results = $dbcore->createGPX->createGPXstructure($results);
		if($labeled){$file_name = $title."_Labeled.gpx";}else{$file_name = $title.".gpx";}
	break;
	...
	case "exp_cid":
		...
		$results = $dbcore->createGPX->CreateApFeatureCollection($CellArray['data']);
		$results = $dbcore->createGPX->createGPXstructure($results);
		if($labeled){$file_name = $title."_Labeled.gpx";}else{$file_name = $title.".gpx";}
	break;
	...
	case "exp_ap_sig":
		...
		$results = $dbcore->createGPX->CreateApFeatureCollection($SigHistArray['data']);
		$results = $dbcore->createGPX->createGPXstructure($results);
		if($labeled){$file_name = $title."_Labeled.gpx";}else{$file_name = $title.".gpx";}
		break;
	...
	case "exp_cell_sig":
		...
		$results = $dbcore->createGPX->CreateApFeatureCollection($CellSigHistArray['data']);
		$results = $dbcore->createGPX->createGPXstructure($results);
		if($labeled){$file_name = $title."_Labeled.gpx";}else{$file_name = $title.".gpx";}
		break;
	...
	case "exp_list":
		...
		$results = $dbcore->createGPX->CreateApFeatureCollection($UserListArray['data']);
		$results = $dbcore->createGPX->createGPXstructure($results);
		if($labeled){$file_name = $title."_Labeled.gpx";}else{$file_name = $title.".gpx";}
	...
	case "exp_cid_list":
		...
		$results = $dbcore->createGPX->CreateApFeatureCollection($CellUserListArray['data']);
		$results = $dbcore->createGPX->createGPXstructure($results);
		if($labeled){$file_name = $title."_Labeled.gpx";}else{$file_name = $title.".gpx";}
	...
	case "exp_user_all":
		...
		$results = $dbcore->createGPX->CreateApFeatureCollection($UserAllList['data']);
		$results = $dbcore->createGPX->createGPXstructure($results);
		if($labeled){$file_name = $title."_Labeled.gpx";}else{$file_name = $title.".gpx";}
		break;
	...
	case "exp_date":
		...
		$results = $dbcore->createGPX->CreateApFeatureCollection($DateList['data']);
		$results = $dbcore->createGPX->createGPXstructure($results);
		if($labeled){$file_name = "date_list_".$title_date."_Labeled.gpx";}else{$file_name = "date_list_".$title_date.".gpx";}
		break;
	...
	case "exp_search":
		...
		$results = $dbcore->createGPX->CreateApFeatureCollection($SearchArray['data']);
		$results = $dbcore->createGPX->createGPXstructure($results);
		if($labeled){$file_name = "Search_".uniqid()."_Labeled.gpx";}else{$file_name = "Search_".uniqid().".gpx";}
		break;
}	
if($xml)
{
