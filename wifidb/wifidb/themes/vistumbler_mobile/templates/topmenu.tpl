				<div>
					<div class="bt-menu-trigger">
						<span></span>
					</div>
					<div class="righttm">
						<p class="center">
							<script>
								if(document.referrer.split('/')[2]==location.hostname){
									document.write('<a class="links" title="Back" style="text-decoration: none;" href="#" onclick="history.go(-1)"><img src="{$themeurl}img/back.png" style="vertical-align: middle;"/><span style="vertical-align: middle; text-decoration: none;">Back</span></a>');
								}
							</script>
							<span class="nowrap">
								<a class="links" title="Home" style="text-decoration: none;" href="{$wifidb_host_url}">
									<img src="{$themeurl}img/home.png" alt="" style="vertical-align: middle;"/>
									<span style="vertical-align: middle; text-decoration: none;">Home</span>
								</a>
							</span>
							<span class="nowrap">
								<a class="links" title="Import File" style="text-decoration: none;" href="{$wifidb_host_url}import/">
									<img src="{$themeurl}img/upload-cloud.png" alt="" style="vertical-align: middle;"/>
									<span style="vertical-align: middle;">Import File</span>
								</a>
							</span>
							<span class="nowrap">							
								<a class="links" title="Map" style="text-decoration: none;" href="{$wifidb_host_url}opt/map.php?func=wifidbmap&labeled=0">
									<img src="{$themeurl}img/map.png" alt="" style="vertical-align: middle;"/>
									<span style="vertical-align: middle;">Map</span>
								</a>
							</span>
							<span class="nowrap">
								<a class="links" title="Stats" style="text-decoration: none;" href="{$wifidb_host_url}stats.php">
									<img src="{$themeurl}img/stats.png" alt="" style="vertical-align: middle;"/>
									<span style="vertical-align: middle;">Stats</span>
								</a>
							</span>
							<span class="nowrap">
								<a class="links" title="Export KMZ" style="text-decoration: none;" href="{$wifidb_host_url}opt/scheduling.php?func=daemon_kml">
									<img src="{$themeurl}img/download-cloud.png" alt="" style="vertical-align: middle;"/>
									<span style="vertical-align: middle;">Export KMZ</span>
								</a>
							</span>
								<span class="nowrap">
								<a class="links" title="Users" style="text-decoration: none;" href="{$wifidb_host_url}opt/userstats.php?func=allusers">
									<img src="{$themeurl}img/users.png" alt="" style="vertical-align: middle;"/>
									<span style="vertical-align: middle;">Users</span>
								</a>
							</span>
							<span class="nowrap">							
								<a class="links" title="Schedule" style="text-decoration: none;" href="{$wifidb_host_url}opt/scheduling.php?func=schedule">
									<img src="{$themeurl}img/schedule.png" alt="" style="vertical-align: middle;"/>
									<span style="vertical-align: middle;">Schedule</span>
								</a>
							</span>
						</p>
					</div>
				</div>