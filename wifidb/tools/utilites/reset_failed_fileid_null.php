...
$sql = "SELECT DISTINCT(wifi_ap.File_ID) FROM wifi_ap INNER JOIN files ON files.id = wifi_ap.File_ID WHERE wifi_ap.fa IS NULL AND files.completed = 1 And File_ID != 1316 ORDER BY wifi_ap.File_ID ASC";
$nil = $dbcore->sql->conn->prepare($sql);
$nil->bindParam(1, $File_ID, PDO::PARAM_INT);
$nil->execute();
while($nilfid = $nil->fetch(1))
{
	$File_ID = $nilfid['File_ID'];
