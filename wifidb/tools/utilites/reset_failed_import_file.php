...
echo "Enter the File Import ID to reset: ";
$handle = fopen ("php://stdin","r");
$File_Import_ID = trim(fgets($handle));
if($File_Import_ID == ''){
    echo "No file ID entered, exiting!\n";
    exit;
}
echo "File Import ID:$File_Import_ID\r\n";
...
If($File_Hash)
{
	...
	$sqlhp = "INSERT INTO files_tmp\n"
		.	 "(file_user, file_orig, file_user, otherusers, notes, title, size, date, hash, converted, prev_ext, type)\n"
			. "SELECT file_user, file_orig, file_user, otherusers, notes, title, size, date, hash, converted, prev_ext, type\n"
