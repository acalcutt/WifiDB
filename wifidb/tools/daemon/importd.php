            ...
            if ($dbcore->checkDaemonKill())# Safely kill script if Daemon kill flag has been set
            {
                ...
                if (!($count <= 8) && preg_match("/Vistumbler VS1/", $return[0]))//make sure there is at least a 'valid' file in the field
                {
                    ...
                    if ($file_hash != @$fileqq['hash'])
                    {
                        ...
                        {
                            $dbcore->verbosed("Start Import of : (" . $file_to_Import['id'] . ") " . $file_name . " For User: " . str_replace("|", "", $user), 1);
                        } else
                        {
                            $dbcore->verbosed("Start Import of Multi-User File (" . $user . "): (" . $file_to_Import['id'] . ") " . $file_name, 1);
                        }
                        ...
                        VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                            ...
                            $prep1->bindParam(10, $id, PDO::PARAM_STR);
