...
function start($command = '')
{
	...
	$console_log = $daemon_config['daemon_log_folder'].$daemon_.'d.log';
	$daemon_script = $config['wifidb_tools']."daemon".$daemon_config['dim'].$daemon_."d.php";
	echo $daemon_script."\r\n".$console_log."\r\n";
	if(!file_exists($daemon_config['daemon_log_folder']))
	{
		if(mkdir($daemon_config['daemon_log_folder']))
		{echo "Made WiFiDB Log Folder [".$daemon_config['daemon_log_folder']."]\r\n";}
		else{echo "Could not make Log Folder [".$daemon_config['daemon_log_folder']."]\r\n";}
	}
	if(!file_exists($daemon_config['pid_file_loc']))
	{
		if(mkdir($daemon_config['pid_file_loc']))
		{echo "Made WiFiDB PID Folder [".$daemon_config['pid_file_loc']."]\r\n";}
		else{echo "Could not make PID Folder [".$daemon_config['pid_file_loc']."]\r\n";}
	}
	
	if (PHP_OS == "WINNT")
	{$cmd = "start ".$daemon_config['php_install']."\php ".$daemon_script." > ".$console_log;}
	else{$cmd = "php ".$daemon_script." -d > ".$console_log." &";}
	...
	if(file_exists($daemon_script))
	{
		...
		#	echo $read;
			sleep(2);
			$pid_file = $daemon_config['pid_file_loc'].$daemon_."d.pid";
			echo $pid_file."\r\n";
...
function status($command = '')
{
	...
	$WFDBD_PID = $daemon_config['pid_file_loc'].$daemon_.'d.pid';
	$os = PHP_OS;
	if ( $os[0] == 'L')
	{
		#echo $os."<br>";
...
function ver()
{
	$ver_db_core = $daemon_config['ver']['wifidb'];
	$ver_Last_Core_Edit = $daemon_config['ver']['Last_Core_Edit'];
	$ver_d = $daemon_config['daemon_ver'];
